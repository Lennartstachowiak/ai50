...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in available_actions:
            q_values[tuple(state), action] = self.get_q_value(state, action)
        best_future_reward = max(
            q_values.items(), key=lambda q_value: q_value[1])[1]
    ...
    def choose_action(self, state, epsilon=True):
        ...
        best_action = max(
            q_values.items(), key=lambda q_value: q_value[1])[0]
        if epsilon and random.random() < self.epsilon:
            random_action = random.choice(list(q_values))
